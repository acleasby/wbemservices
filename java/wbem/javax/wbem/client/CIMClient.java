...
public class CIMClient implements CIMOMHandle {
	...
	private class IndicationDeliverer extends Thread {
		...
		List l = new ArrayList(listenerList);
	...
	csc = ClientSecurityFactory.createClientSecurity(nameSpace,
		principal, credential, null, this.getProtocol());
	...
	return (api.enumNameSpace(version, checkNameSpace(path), path, 
	    deep)).elements();
    } // enumNameSpace
	...
	return (enumerateClasses(path, deep, localOnly, includeQualifiers, 
		false));
	...
	Vector v = api.enumerateClasses(version, checkNameSpace(path), path, 
					deep, localOnly, includeQualifiers, 
					includeClassOrigin);
	return v.elements();
	...
	return (api.enumerateClassNames(version, checkNameSpace(path), path, 
	    deep)).elements();
    } // enumerateClassNames
	...
	return (api.enumerateInstanceNames(version, checkNameSpace(path), 
	    path)).elements();
	...
	return enumerateInstances(path, deep, localOnly, includeQualifiers, 
		    	   	  false, null);
	...
	return enumerateInstances(path, deep, localOnly, 
				  includeQualifiers,
	...
	Vector v = api.enumerateInstances(version, checkNameSpace(path), path, deep,
	...
	return getClass(name, localOnly, includeQualifiers, includeClassOrigin,
			null);
	...
	return api.getClass(version, checkNameSpace(name), name, localOnly,
	...
	return getInstance(name, localOnly, includeQualifiers, 
			   includeClassOrigin, null);
	...
	return api.getInstance(version, checkNameSpace(name), name, localOnly,
			   includeQualifiers, includeClassOrigin, propertyList);
    } // getInstance
	...
	return api.invokeMethod(version, checkNameSpace(name), name, methodName,
		    inArgs, outArgs);
     ...
     * </pre>                     
     */
    public synchronized void setClass(CIMObjectPath name,
     ...
     * </pre>   
     */
    public synchronized void setInstance(CIMObjectPath name,
	    CIMInstance ci) throws CIMException {
    ...
    public void setInstance(CIMObjectPath name, CIMInstance ci,    
    	...
    	api.setInstance(version, checkNameSpace(name), name, ci, includeQualifiers,
    	  propertyList);
	...
	return api.getProperty(version, checkNameSpace(name), name, propertyName);
	...
	api.setProperty(version, checkNameSpace(name), name, propertyName, newValue);
     ...
     * </pre>
     */
    public synchronized Enumeration execQuery(CIMObjectPath path,
	    String query,
	...
	Vector v = api.associators(version, checkNameSpace(objectName), objectName,
    ...
    public Enumeration associatorNames(CIMObjectPath objectName, 
        ...
        return (api.associatorNames(version, checkNameSpace(objectName), 
            objectName, assocClass, resultClass, role, resultRole)).elements();
    } // associatorNames
	...
	Vector v = api.references(version, checkNameSpace(objectName), objectName, resultClass, 
	...
	return (api.referenceNames(version, checkNameSpace(objectName), objectName,
			    resultClass, role)).elements();
    } // referenceNames
	...
	Debug.trace1("Starting CIMClient version " +
