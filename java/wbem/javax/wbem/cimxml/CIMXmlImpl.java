...
class CIMXmlImpl implements CIMXml, CIMXmlUtil {
	...
	NodeTreeWalker walker = new NodeTreeWalker(node);
		...
		op += ((Element)node).getAttribute(XmlInstance.NAME);
	    } else if (nodename.equals(CIMXml.CLASSNAME)) {
		op += ":" + ((Element)node).getAttribute(XmlInstance.NAME);
	    } else if (nodename.equals(CIMXml.INSTANCENAME)) {
		op += ":" + ((Element)node).getAttribute(CIMXml.CLASSNAME);
		...
		value = buf.toString();
		op += (type.equals("string") ? "\"" + value + "\"" : value);
	    } else if (nodename.equals(CIMXml.VALUE_REFERENCE)) {
		isSingleton = false;
		op += "=" + objectPath(walker.nextNode());
	    }
	}
	op += (isSingleton ? "=@" : "");
	String returnStr;
	try {
	    returnStr = URLEncoder.encode(op, "UTF-8");
