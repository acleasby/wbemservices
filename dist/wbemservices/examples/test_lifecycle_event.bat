...
copy %EXAMPLES%\client\event\*_Provider.class %CIMOMLIB%
...
set MOF=TestIntrinsicEvents.mof
cd %WBSERBIN%
call mofcomp -v -u %USER% -p %PASSWD% %EXAMPLES%\client\event\%MOF%
cd %EXAMPLES%

:CREATE
echo Run test_lifecycle_listener %USER% %PASSWD% %PROTOCOL% %HOST% in another DOS window
echo When test_lifecycle_listener shows: Waiting for event CIM_InstCreation
pause
...
@echo off
echo When test_lifecycle_listener shows: Waiting for event CIM_InstModification
pause
...
@echo off
echo When test_lifecycle_listener shows: Waiting for event CIM_InstRead
pause
...
@echo off
echo When test_lifecycle_listener shows: Waiting for event CIM_InstMethodCall
pause

set PROGRAM=InvokeCIMMethod
set CLASS=Class_E
...
@echo off
echo When test_lifecycle_listener shows: Waiting for event CIM_InstDeletion
pause
...
%JAVA% %CLASSPATH% %PROGRAM% %HOST% %USER% %PASSWD% %CLASS% keyE=E.1 %PROTOCOL% 
@echo off
echo When test_lifecycle_listener completes
pause
