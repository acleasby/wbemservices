...
if [ "$3" = "" ]; then
	echo "USAGE: sh test_namespace.sh user password protocol [host]"
...
CLASSPATH=".:$WBEM"
PROGRAM=TestCreateNameSpace
cd $EXAMPLES/client/namespace
echo "**********************************************"
echo "Running $PROGRAM on $HOST/root over $PROTOCOL"
echo "**********************************************"
echo $JAVA $CLASSPATH $PROGRAM $HOST $USER $PASSWD $PARENT $CHILD $PROTOCOL 
$JAVA $CLASSPATH $PROGRAM $HOST $USER $PASSWD $PARENT $CHILD $PROTOCOL 

PROGRAM=TestEnumNameSpace
cd $EXAMPLES/client/namespace
echo "**********************************************"
echo "Running $PROGRAM on $HOST/root over $PROTOCOL"
echo "**********************************************"
