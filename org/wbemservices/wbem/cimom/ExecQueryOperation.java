...
class ExecQueryOperation extends CIMOMOperation {
    ...
    ExecQueryOperation(CIMOMServer cimom, ServerSecurity ss,
			CIMExecQueryOp cimop, String version) {

	super(cimom, ss, cimop.getNameSpace(), version);
    	this.op = cimop.getModelPath();
	this.query = cimop.getQuery();
	this.queryLanguage = cimop.getQueryLanguage();
    }
	...
	try {
	    ...
	    // result = cimom.intexecQuery(ns, op, query, queryLanguage, false);

	    result = cimom.execQuery(version, ns, op, query,
	    				queryLanguage, ss);
